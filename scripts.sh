...
alias aoc-load="mkdir -p $AOC; cd $AOC; touch p1.py; touch p2.py; curl --cookie "session=$AOC_COOKIE" "$(echo `date +https://adventofcode.com/%Y/day/%d/input` | sed 's/\/0/\//g')" > inp.txt"
